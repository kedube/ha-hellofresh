...
def test_account_menu_data_skips_past_weeks() -> None:
    """The planning-menu endpoint must not be queried for past weeks.

    For a past week the endpoint serves a substitute menu that would overwrite the real
    delivered meals. The web app only requests the current/upcoming weeks, so a past-dated
    account week is skipped entirely and never reaches /gw/my-deliveries/menu.
    """
    client = HelloFreshClient(session=None)  # type: ignore[arg-type]
    subscription = HelloFreshSubscription(
        subscription_id="6959884",
    ...
    past_week = HelloFreshWeek(
        week_id="2020-W07",  # safely in the past regardless of the test clock
        display_name="Feb 9 - Feb 15",
        subscription_id="6959884",
        delivery_date=date(2020, 2, 13),
        raw={
            "deliveryOption": {"handle": "US-1-0800-2000"},
            "product": {"handle": "US-CBU-3-2-0"},
        },
    ...
    future_week = HelloFreshWeek(
        week_id="2099-W30",  # safely in the future regardless of the test clock
        display_name="Jul 20 - Jul 26",
        subscription_id="6959884",
        delivery_date=date(2099, 7, 22),
        raw={
            "deliveryOption": {"handle": "US-1-0800-2000"},
            "product": {"handle": "US-CBU-3-2-0"},
        },
    ...
    requested_menu_weeks: list[str | None] = []

    async def fake_api_get(path: str, params=None):
        if path == "/gw/api/subscriptions/6959884/product_options":
            raise HelloFreshError("not available")  # fall back to the subscription preset
        if path == "/gw/my-deliveries/menu":
            requested_menu_weeks.append((params or {}).get("week"))
        return DummyResponse()
    ...
    async def fake_response_json(_response):
        return {
            ...
            "week": "2099-W30",
            "meals": [
                {"index": 1, "recipe": {"id": "r-future", "name": "Future Week Dish"}},
            ],
        }
    ...
    loop.run_until_complete(
        client._async_get_account_menu_data([subscription], [past_week, future_week])
    )

    # Only the future week reached the planning-menu endpoint; the past week was skipped.
    assert requested_menu_weeks == ["2099-W30"]
