...
| [docs/dashboard.md](docs/dashboard.md) | The dashboard's views and all eight Lovelace cards: options, features, screenshots |
...
- **Tracking** — the packaged [Delivery tracking card](docs/dashboard.md#delivery-tracking-card) paired with a built-in map card of the driver's live position. **Netherlands accounts only** — it rides HelloFresh's own delivery-fleet tracker, which doesn't exist where third-party carriers deliver; other accounts see a "not available in your region" notice and can delete the view.
