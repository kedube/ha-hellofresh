...
- **Missing Ingredients** — a built-in **to-do list card** over `todo.<prefix>_prep_list`: the pantry staples that HelloFresh does **not** ship — salt, oil, butter, eggs — for the selected meals of your **next two deliveries**, so one shopping trip can cover both boxes. Each item is due on its own box's delivery date, which is how the card groups the two weeks; quantities are added up per unit within a week (two meals wanting 2 tablespoon of butter show **4 tablespoon**) but never merged across weeks. Check them off as you shop — when a box arrives its items drop off and the following week's carry over, ticks included. It is a projection of those weeks' selections, so it can't be added to or deleted from, only ticked. See [`todo.prep_list`](docs/entities.md).
