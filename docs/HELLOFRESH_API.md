...
**Tracey live tracking** (`HelloFreshTraceyCoordinator`, Netherlands entries only) polls the unauthenticated `https://europe-west1-hellofresh-prod.cloudfunctions.net/c_hf_getTraceyData` endpoint with the token from the active `hftrack.nl` tracking URL. The active delivery cadence is the Netherlands-only `delivery_tracking_refresh_interval_seconds` option (default 300, range 60–3600); outside an active Tracey delivery it falls back to a 30-minute idle cadence. Other countries never mount this coordinator, never expose the option, and `hellofresh.get_delivery_tracking` returns `available: false`.
