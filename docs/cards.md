...
For the ready-made dashboard that assembles these, see
...
| `logo` | all except meal planner | Set `true` to show the bundled HelloFresh logo in the header. |
...
- **Sold-out meals** — a meal HelloFresh has marked sold out is greyed out with a **Sold out** ribbon. This is **advisory, not enforced**: the tile stays tappable and `select_meals` still submits, logging a warning. See the note below for why.
...
- **Meal filters** (current & upcoming weeks) — a filter bar to narrow the menu by **protein** (Beef, Poultry, Pork, Seafood, Lamb, Veggie — tap any combination, or **All** to clear), by **dietary category** (GLP-1 Support, Carb Conscious, High Protein, Under 650 Calories, Gluten-Free Friendly, Sodium Smart, Low Added Sugar, High Fiber, Mediterranean, Under 15/20/30 Minutes — the same categories as HelloFresh's own menu filters and Health Conscious section), by **highlight** (New, Bestsellers, Cooked Before — read from HelloFresh's own badges, tags and delivered-count, combining as a union like the site's browse rows), by **menu section** (the website's own sections — This Week's Menu, Health Conscious Menu, Family Menu, Bestsellers, Your Top Recipes, Order It Again, … — single-select, drawn from each week's menu payload so they track whatever HelloFresh publishes), and to **hide variants** so only the base meal of each dish shows (the 2× protein, protein-swap and veggie-swap versions are collapsed away). Protein chips widen the selection (a meal has one protein), while each dietary chip adds a constraint — pick High Protein *and* Under 30 Minutes to see meals that are both. Dietary categories match the tags HelloFresh puts on each recipe (under every spelling it has used — e.g. GLP-1 meals are tagged "GLP-1 Support", "GLP-1 Friendly" or "GLP-1 Balance" depending on the season), and the calorie/time categories also accept an untagged meal whose own numbers qualify. Your currently selected meals always stay visible regardless of the filter. The bar is hidden on weeks past the [**Full menu history** window](../README.md#options) (which just show what was delivered); a just-delivered week still has its full menu, so it keeps the filters.
...
- **Search field** — a text box under the header that filters the current view as you type, matching recipe names and headlines. It searches what the card has **loaded** (the selected category, the Cookbook, or Top rated — up to `limit` recipes), not the whole 10,000-recipe catalog: HelloFresh exposes no text-search endpoint, so switch categories to search elsewhere.
