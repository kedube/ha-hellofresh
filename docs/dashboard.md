...
- **Payment-method warning built in** — when HelloFresh's own check flags the card on file as expiring (amber) or expired (red), a banner says so at the top of the card, with the expiry month, so a box does not silently fail to ship. Backed by `binary_sensor.payment_method_expiring`.
