...
- **Paces itself** — refetches on the Netherlands-only **Delivery tracking refresh interval** while a delivery is on the road (default 300 seconds), every 15 minutes when idle, and not at all for unsupported regions (one `available: false` answer is definitive). The integration additionally throttles the underlying endpoint fetch to once a minute across all open dashboards. Outside a delivery window the card shows your next delivery date, so it's never just blank.

It reads the response-returning `hellofresh.get_delivery_tracking` service — each card refresh is a **live** fetch of the tracking endpoint on the configured live-tracking cadence. Read-only.
