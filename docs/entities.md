...
| `todo.prep_list` | Shown as **Prep list**. The pantry staples that HelloFresh does **not** ship — salt, oil, butter, eggs — for the selected meals of your **next two deliveries**: the box on its way and the one after it, so a single shopping trip can cover both. Add it to a **To-do list** card; each item is due on its own box's delivery date, which is how the card separates the two weeks. It is a projection of those weeks' selections, not a general shopping list: items appear and vanish as you change your meals, so the list cannot be added to or deleted from, only checked off. Amounts are summed per unit **within** a week (two meals wanting 2 tablespoon of butter show **4 tablespoon**) but never across weeks — each box is its own trip — and amounts in genuinely different units are shown side by side rather than converted. When a box arrives its items drop off; the following week's items, including anything already ticked, carry over. Skipped weeks ship nothing and are passed over. |
