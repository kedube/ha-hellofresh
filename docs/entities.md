...
| Name | Entity | Description |
| --- | --- | --- |
| Next delivery total price | `sensor.next_box_total_price` | Sum of all charges for the next upcoming delivery date, across every billing item for that date. Monetary device class; the unit reflects the subscription currency. Attributes carry a `price_breakdown` for that box (subtotal, shipping, discount, tax, coupon) from `/gw/calculate`. Its `billed_discount` attribute is what the billing ledger actually took off that delivery's charges (a wallet voucher realized as a coupon line), which the calculate split in `price_breakdown` never shows. |
| Next box discount | `sensor.next_box_discount` | The **weekly discount** HelloFresh's wallet will apply to the next box that ships — e.g. `10.00` for a "$10 off premium meals" voucher (`POST /gw/customer-wallet/v2/benefit-distribution`, the same call the deliveries page makes). Monetary device class; the unit is the account currency. Attributes: `label` ("$10 off premium meals"), `applies_to` (`premiumSurcharge`, `shipping`, …), `amount_type` (`fixed` or `percent`), `voucher_code`, `one_time`, `expires_at`, `week_id`, and `benefits` (every promise on the account with the upcoming weeks it is available for). Unknown when no promise covers an upcoming box. The voucher code is redacted from diagnostics exports. |
...
| `binary_sensor.payment_method_expiring` | Shown as **Payment method expiring**. `True` when HelloFresh reports the card on file as **expiring or already expired** (`POST /gw/payments/v1/checktokenstatus`, the same check the website runs) — the most common way a box silently fails to ship. Attributes: `expiring`, `expired`, `card_type`, `card_brand` (visa, mastercard, …), `card_last4` (the last four digits, the only part of the number HelloFresh returns), `card_provider`, `card_expiry` (`YYYY-MM`). The billing address in that response is dropped at parse time and never stored, and the digits are redacted from diagnostics exports. `Problem` device class; unavailable until the payments gateway has answered. |
