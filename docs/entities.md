...
| `todo.prep_list` | Shown as **Prep list**. The pantry staples that HelloFresh does **not** ship — salt, oil, butter, eggs — for the selected meals of your **next delivery**. Add it to a **To-do list** card. Quantities are summed per unit (two meals wanting 2 tablespoon of butter show **4 tablespoon**); amounts in genuinely different units are shown side by side rather than converted, and items are due on the delivery date. It is a projection of that week's selection, so it cannot be added to or deleted from, only checked off. Attributes carry `week_id` and `delivery_date` for dashboard headings. |
